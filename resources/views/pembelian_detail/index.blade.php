...
@push('scripts')
    <script>
        ...
        // kalau kosong, paksa jadi 0
        if (val === '' || isNaN(val)) {
            val = 0;
            $(this).val(0);
        }

        let diskon = parseInt(val);
        ...
        .done(response => {
            $('#kode_produk').val('').focus(); // reset + fokus lagi
        ...
        $.get(`{{ url('/pembelian/cari') }}/${kode}`)
            .done(response => {
                if (response.success) {
                    $('#id_produk').val(response.id_produk);
                    tambahProduk(); // langsung tambah ke tabel
                } else {
